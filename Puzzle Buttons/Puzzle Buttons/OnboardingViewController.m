...
- (instancetype)initWithContents:(NSArray *)contents {
    ...
    [self.skipButton.titleLabel setFont:[UIFont fontWithName:@"HelveticaNeue-Light" size:14]];
...
- (void)viewDidLoad {
    ...
    if (_baseColor) {
        self.pageControl.currentPageIndicatorTintColor = _baseColor;
    }else{
...
//    _pageVC.view.frame = self.view.frame;
    [_pageVC.view setFrame:CGRectMake(0, 20, self.view.frame.size.width, self.view.frame.size.height)];
    
    UIView* statusBackground = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 20)];
    statusBackground.backgroundColor = _baseColor;
    ...
    if (self.allowSkipping) {
        self.skipButton.frame = CGRectMake(0,
                                           CGRectGetMaxY(self.view.frame) - self.underPageControlPadding - kSkipButtonHeight,
                                           kSkipButtonWidth,
                                           kSkipButtonHeight);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_currentPage == [self.viewControllers lastObject]){
        CGFloat totalPercent = fabs(scrollView.contentOffset.x / self.view.frame.size.width);
        if (totalPercent > 1.0){
            [self handleSkipButtonPressed]; //to end the onboarding with a swipe
        }
    }
